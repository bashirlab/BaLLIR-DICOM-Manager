...
class ReadMultiImageLabelPair(ReadImageLabelPair):
    ...
    def get_multi_label_array(
        self, pixel_data_1: np.array, pixel_data_2: np.array
    ) -> np.array:
        """Return array with overlap=1, pixel_data_1=2, pixel_data_2=3."""
    ...
    def build_rgb_overlay(self, **kwargs) -> np.array:
        ...
        multi_label_array = self.get_multi_label_array(
            self.read_pair_1.read_dicom_label.arr, self.read_pair_2.read_dicom_label.arr
        )
        self.foreground = np.copy(multi_label_array)
        self.foreground[multi_label_array > 0] = 1

        rgb_array = self.dampen_mask_regions(rgb_array, self.foreground)
