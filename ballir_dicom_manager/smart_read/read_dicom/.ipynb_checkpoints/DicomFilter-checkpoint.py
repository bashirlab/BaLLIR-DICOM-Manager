...
class DicomFilter(ABC):
    ...
    def get_position(self, dicom_file):
...
class RemoveSliceOverlapFilter(DicomFilter):
    ...
    def get_step_size(self, dicom_file) -> float:
    ...
    def get_step_sizes(self, dicom_files) -> List[float]:
        return [self.get_step_size(file) for file in dicom_files] 

    def get_position(self, dicom_file) -> float:
        if hasattr(dicom_file, 'ImagePositionPatient'):
            return dicom_file.ImagePositionPatient[2]
        elif hasattr(dicom_file, 'SliceLocation'):
            return dicom_file.SliceLocation
    ...
    def get_positions(self, dicom_files: List[dcm.dataset.Dataset]) -> List[float]:
        return [self.get_position(file) for file in dicom_files]

    def approximate_best_locations(self, dicom_files) -> List[float]:
        average_step: float = np.mode(self.get_step_sizes(dicom_files))
        slice_positions = self.get_step_sizes(dicom_files)
