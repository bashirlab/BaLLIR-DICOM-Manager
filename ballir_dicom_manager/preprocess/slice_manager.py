...
class SliceManager:
    ...
    def return_best_positions(self, dicom_slice_positions: list, step_size: float) -> list:
        position_offset = self.return_position_offset(dicom_slice_positions, step_size)
        best_range = self.return_best_range(dicom_slice_positions, position_offset, step_size)
        return [best_range[0] + step_size*slice_num for slice_num in range(1+int((best_range[1]-best_range[0])/step_size))]

    def return_missing_positions(self, dicom_slice_positions: list, step_size: float) -> list:
        best_positions = self.return_best_positions(dicom_slice_positions, step_size)
        return [loc for loc in best_positions if not loc in dicom_slice_positions]

    def return_extra_positions(self, dicom_slice_positions: list, step_size: float) -> list:
        best_positions = self.return_best_positions(dicom_slice_positions, step_size)
        return [loc for loc in dicom_slice_positions if not loc in best_positions]
    
    def return_position_offset(self, dicom_slice_positions: list, step_size: float) -> float:
    ...
    def return_best_range(self, dicom_slice_positions: list, position_offset: float, spacing: float) -> tuple:
    ...
    def return_next_best_positions(self, dicom_slice_positions: list, best_positions: list) -> list:
        return [self.closest(dicom_slice_positions, loc) for loc in best_positions]
    
#     def return_step_size(self, dicom_slice_positions: list) -> float:
#         return self.most_common([round(slice_position - dicom_slice_positions[num-1], 5) for num, slice_position in enumerate(dicom_slice_positions)])
    ...
    def return_step_size(self, dicom_files: list) -> float:
