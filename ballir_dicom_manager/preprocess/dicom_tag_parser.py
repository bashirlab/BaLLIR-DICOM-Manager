...
class DicomTagParser:
    ...
    def get_dicom_slice_spacing(self, dicom_files):
        ...
        assert len(dicom_slice_spacing) == 1, f'{len(dicom_slice_spacing)} spacings detected: {dicom_slice_spacing}'
    ...
    def get_dicom_pixel_spacing(self, dicom_files):
        ...
        if not 'PixelSpacing' in self.allow:
            assert len(dicom_pixel_spacing) == 1, f'{len(dicom_pixel_spacing)} spacings detected: {dicom_pixel_spacing}'
