...
class ProcessResultsNifti(ReadPairNifti):
    ...
    def __init__(self, scan, mask_gt, mask_alg, colors = ['#06b70c', '#2c2cc9', '#eaf915'], transparency = 0.3, outline = False, legend_size = False):
        ...
        if outline:     
            ...
            for arr_slice in range(self.mask.arr.shape[2]):
                mask_gt_erode[..., arr_slice] = scipy.ndimage.binary_erosion(self.mask.arr[..., arr_slice], structure = np.ones((outline, outline)))
            ...
            for arr_slice in range(self.mask_alg.arr.shape[2]):
                mask_alg_erode[..., arr_slice] = scipy.ndimage.binary_erosion(self.mask_alg.arr[..., arr_slice], structure = np.ones((outline, outline)))
